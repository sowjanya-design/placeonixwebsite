...
export default function ProgramCard({ course }: { course: Course }) {
  ...
  const tag = isCombo
    ? "Combo Package"
    : DEMAND_TAG[course.slug] || "Technology";
  ...
  if (isHighlight) {
    return (
      <Link
        className="prog-card sap-highlight reveal"
      ...
      >
        <div className="sap-inner">
          ...
          <div>
            ...
            >
              Highest Demand — 5 Modules
            </span>
            <h3>{title}</h3>
            <p>{shortLead}</p>
            <div className="prog-meta">
              <span>⏱ {course.badges[0]}</span>
              <span>☁️ SAP Cloud</span>
              <span>💻 Modern Dev Stack</span>
  ...
  return (
    ...
    >
      ...
      <div className="prog-meta">
        <span>⏱ {course.badges[0]}</span>
        {!isCombo && iconFor(course.slug) && (
          <span>{iconFor(course.slug)}</span>
        )}
      </div>
      <span className="prog-card-link">View course page →</span>
    </Link>
  );
}
