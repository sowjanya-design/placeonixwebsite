...
export async function POST(req: NextRequest) {
  ...
  // Strip CR/LF so a crafted name can't inject extra headers into the subject line.
  const safeFullName = String(fullName).replace(/[\r\n]+/g, " ").trim();

  const adminHtml = `
    <h2>New Placeonix Application</h2>
    <p><strong>Enquiry Type:</strong> ${intent === "demo" ? "Demo Class Enquiry" : "Course Enquiry"}</p>
  ...
  try {
    ...
    await Promise.all([
      transporter.sendMail({
        ...
        subject:
          intent === "demo"
            ? `Demo Class Enquiry - ${safeFullName}`
            : `Course Enquiry - ${safeFullName}`,
        html: adminHtml,
      }),
      transporter.sendMail({
